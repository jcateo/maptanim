            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`px-4 py-2 border rounded-lg font-medium transition-colors ${largeText ? 'text-base' : 'text-sm'} ${
                    highContrast ? 'bg-gray-800 border-gray-600 text-white hover:bg-gray-700' : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100'
                  }`}
                >
                  Update
                </button>
          ...
          <div className={`border rounded-xl shadow-sm overflow-hidden ${highContrast ? 'bg-black border-gray-600' : 'bg-white border-gray-200'}`}>
            ...
            <div className="p-6">
              <p className={`mb-4 ${highContrast ? 'text-gray-300' : 'text-gray-600'} ${largeText ? 'text-base' : 'text-sm'}`}>
                MapTanim Phase 13 introduces full bilingual support. Select your preferred language below.
              </p>
              <div className="grid grid-cols-2 gap-4 max-w-sm">
                <label 
                  onClick={() => setLanguage('en')}
                  className={`border-2 rounded-lg p-4 cursor-pointer text-center relative overflow-hidden transition-all ${
                    language === 'en' ? 'border-brand-500 bg-brand-50' : highContrast ? 'border-gray-600 hover:border-gray-400' : 'border-gray-200 hover:bg-gray-50'
                  }`}
                >
                  {language === 'en' && <div className="absolute top-2 right-2 w-3 h-3 bg-brand-500 rounded-full"></div>}
                  <span className={`block font-bold mt-1 ${
                    language === 'en' ? 'text-brand-900' : highContrast ? 'text-white' : 'text-gray-700'
                ...
                <label 
                  ...
                  className={`border-2 rounded-lg p-4 cursor-pointer text-center relative overflow-hidden transition-all ${
                    language === 'fil' ? 'border-brand-500 bg-brand-50' : highContrast ? 'border-gray-600 hover:border-gray-400' : 'border-gray-200 hover:bg-gray-50'
                  }`}
                >
                  {language === 'fil' && <div className="absolute top-2 right-2 w-3 h-3 bg-brand-500 rounded-full"></div>}
                  <span className={`block font-bold mt-1 ${
                    language === 'fil' ? 'text-brand-900' : highContrast ? 'text-white' : 'text-gray-700'
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`mt-0.5 ${highContrast ? 'text-gray-300' : 'text-gray-500'} ${largeText ? 'text-sm' : 'text-xs'}`}>Increases contrast for easier reading.</p>
                </div>
                <div 
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`mt-0.5 ${highContrast ? 'text-gray-300' : 'text-gray-500'} ${largeText ? 'text-sm' : 'text-xs'}`}>Scales typography across the application.</p>
                </div>
                <div 
